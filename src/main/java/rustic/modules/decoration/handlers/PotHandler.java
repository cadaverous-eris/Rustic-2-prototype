...
import rustic.modules.decoration.MessagePotMeta;
...
public class PotHandler {
	...
	public static void onPotMouseWheel(MouseEvent event) {
		...
		if (player != null) {
			if (event.getButton() == 2 && !event.isButtonstate() && player.getHeldItemMainhand().getItem() == Item.getItemFromBlock(Pots.pot)) {
			...
			if (player.isSneaking() && event.getDwheel() != 0 && player.getHeldItemMainhand().getItem() == Item.getItemFromBlock(Pots.pot)) {
