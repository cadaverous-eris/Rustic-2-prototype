...
public class BlockReceptacle extends BlockFluidStorage {
	...
	public IBlockState getStateFromMeta(int meta) {
		return getDefaultState().withProperty(PIPE_DIR, EnumFacing.getFront(5 - (meta & 3)));
	}

	@Override
	public int getMetaFromState(IBlockState state) {
		return 5 - ((EnumFacing) state.getValue(PIPE_DIR)).getIndex();
	}
	...
	*/
	...
	public void neighborChanged(IBlockState state, World world, BlockPos pos, Block block, BlockPos fromPos) {
		if (state.getValue(CONNECTED) && fromPos.equals(pos.offset(state.getValue(PIPE_DIR)))) {
			if (world.getBlockState(fromPos).getBlock() != RegistryManager.alembic) {
